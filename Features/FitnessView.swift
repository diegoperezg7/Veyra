...
struct ExerciseLibrary: View { @Environment(AppModel.self) private var model; var onSelect: ((ExerciseDefinition) -> Void)?; @State private var search = ""; var body: some View { List(model.exercises.filter { search.isEmpty || $0.name.localizedCaseInsensitiveContains(search) }) { exercise in if let onSelect { Button(exercise.name) { onSelect(exercise) } } else { NavigationLink(exercise.name) { Text(exercise.instructions).padding() } } }.searchable(text: $search).navigationTitle(L("exerciseLibrary")) } }
